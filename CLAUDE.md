...
**Build in the order in `SPEC.md` §11.** Each milestone is independently useful. Do not build UI before fingerprinting is proven by the property tests — a brittle fingerprint makes the tool worse than nothing, and you will not discover that through a UI.
...
- Build order: generator in `SPEC.md` §11; monitor and posture checks in `MONITORING.md` §10.
